...
# Aquí comienza el PRIMER desafío

# Creación de la ruta al archivo y una lista para leer los datos
archivo = Path("C:\\Users\\Usuario\\Desktop\\Butucamopo\\MisterHacker\\Python challenge\\PyBank\\Resources\\budget_data.csv")
...
with open(archivo, 'r') as file:
    ...
    for row in reader:
        ...
        # Obtener el primer y último valor para el cálculo del cambio
        if meses == 1:
            primer_valor = int(row[1])
        ultimo_valor = int(row[1])

    cambio_total = ultimo_valor - primer_valor
    promedio_cambio = cambio_total / (meses - 1)

    print("Análisis financiero")
    print("-------------------------")
    print(f"Total de meses: {meses}")
    print(f"Total: ${total_profit}")
    print(f"Cambio total: ${cambio_total}")
    print(f"Promedio de cambio: ${promedio_cambio}")

    # Aquí termina el PRIMER desafío

    # Aquí comienza el SEGUNDO desafío

    # Establecer la ruta al archivo
    archivo2 = Path("C:\\Users\\Usuario\\Desktop\\Butucamopo\\MisterHacker\\Python challenge\\PyPoll\\Resources\\election_data.csv")
    ...
    with open(archivo2, 'r') as file2:
        ...
        for row in reader2:
            ...
            # Verificar si el candidato ya existe en el diccionario
            if candidato in candidatos:
                candidatos[candidato] += 1
            else:
                candidatos[candidato] = 1

            # Verificar si el candidato tiene más votos que el actual ganador
    ...
    for candidato, votos in candidatos.items():
        ...
        print(f"{candidato}: {porcentaje:.3f}% ({votos})")
    print("-------------------------")
    print(f"Ganador: {ganador}")

    # Aquí termina el SEGUNDO desafío

# Exportar los resultados a un archivo de texto
new_file = "C:\\Users\\Usuario\\Desktop\\Butucamopo\\MisterHacker\\Python challenge\\budget_results.txt"

with open(new_file, "w") as results:
    results.write("Análisis financiero\n")
    results.write("-------------------------\n")
